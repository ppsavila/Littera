...
export function ErrorMarkerToolbar() {
  const { isErrorMode, selectedErrorCode, setSelectedErrorCode } = useErrorMarkerStore()
  const [activeCompetency, setActiveCompetency] = useState<number>(1)

  if (!isErrorMode) return null
          ...
          {errorTypes.map((et) => {
            ...
            return (
              <button
                ...
                style={
                  active
                    ? { background: 'var(--littera-rose)', color: '#fff', border: '1px solid transparent' }
                    : { background: 'rgba(255,255,255,0.85)', color: 'var(--littera-rose)', border: '1px solid rgba(190,18,60,0.30)' }
                }
              >
                <span
                  className="inline-flex items-center justify-center w-6 h-4 rounded text-[10px] font-bold"
                  style={{ background: active ? 'rgba(255,255,255,0.25)' : severityBg, color: '#fff' }}
                >
                  {et.code}
                </span>
